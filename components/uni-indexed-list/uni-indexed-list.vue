			...
			setList() {
				...
				this.options.forEach((value, index) => {
					...
					let items = value.data.map(item => {
						...
						obj['nicakname'] = item.nicakname
						obj['profession'] = item.profession
						// obj['mid'] = value.data.mid
						// obj['name'] = item
						// obj['itemIndex'] = index
						index++
				...
				uni.createSelectorQuery()
					...
					.exec(ret => {
						// console.log(ret)
						this.winOffsetY = ret[0].top+100
						this.winHeight = ret[0].height-200
				...
				dom.getComponentRect(this.$refs['list'], (res) => {
					console.log(res)
					this.winOffsetY = res.size.top+100
					this.winHeight = res.size.height-200
					this.itemHeight = this.winHeight / this.lists.length
				})
				// #endif
			},
