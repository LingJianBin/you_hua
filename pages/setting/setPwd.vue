<template>
	<view class="content">
		...
		<view class="pwd">
			<view class="oldpwd">
				<input type="number" v-model="verifycode" placeholder="请输入验证码" placeholder-style="color: #999;" maxlength="6" />
			...
			<view class="oldpwd">
				<input type="password" v-model="pwd" placeholder="请输入新密码" placeholder-style="color:#999;" />
			</view>
			<view class="oldpwd">
				<input type="password" v-model="repwd" placeholder="请确认新密码" placeholder-style="color:#999;" />
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			submit() {
				var self = this
				uni.request({
					url: service.api.setting.changePwd,
					method: 'POST',
					header: {
						Authorization: 'Bearer ' + uni.getStorageSync('token1')
					},
